...
class WeatherForecastController():
    ...
    def _draw_forecast(self, forecast: WeatherData) -> None:
        ...
        self._draw_temperature(18, 2, forecast.night_temp_c)
        self._draw_temperature(18, 14, forecast.day_temp_c)
    ...
    def _draw_temperature(self, x: int, y: int, temperature: int) -> None:
        ...
        if temperature < -10:
            self._draw_negative_sign(x - 4, y + 4, colour)
        elif temperature < 0:
            self._draw_negative_sign(x + 2, y + 4, colour)

    def _draw_negative_sign(self, x: int, y: int, colour: RGBColour) -> None:
        self._canvas.draw_horizontal_line(x, y, 3, colour, layer=Layer.Middle)
    ...
    def _draw_weather_symbol(self, x: int, y: int, symbol_id: str):
        if symbol_id not in self._weather_symbol_sprites:
